...
const { sendDiscordNotification, sendPayloadToWebhook } = require('../services/notificationService');
    ...
    try {
      ...
      for (const row of rows) {
        ...
        try {
          ...
          // Gửi thông báo Discord và webhook-test
          console.log(`Gửi thông báo cho trackingId: ${trackingId}`);
          await sendDiscordNotification(trackingId, latestData.state_name, latestData.date);
          await sendPayloadToWebhook(trackingId, latestData.state_name, latestData.date);
  
          console.log(`Đã gửi thông báo cho trackingId: ${trackingId}`);
